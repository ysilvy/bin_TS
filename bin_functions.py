...
def bin_TS(S,T,vol,sbins,tbins):
    ...
    vol: Volume xarray dataset (volume at each cell grid) 3D
...
def select_basin(ds,mask):
    ...
    # Southern Ocean
    ds_out = xr.where(mask.nav_lat<=-35,ds,np.nan).where(ds.data<1.e19)
    ds_out['region']='southern_ocean'
    # Atlantic
    ds_atl = xr.where(mask.atlmsk==1,ds,np.nan).where(mask.nav_lat>-35)
    ds_atl['region']='atlantic'
    ds_out = xr.concat([ds_out,ds_atl], dim='region')
    # Pacific
    ds_pac = xr.where(mask.pacmsk==1,ds,np.nan).where(mask.nav_lat>-35)
    ...
    ds_ind = ds_ind.assign_coords(region='pacific').expand_dims('region')
